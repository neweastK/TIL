  ...
  <div class="container">
    <ul class="nav nav-tabs">
      ...
      <table class="table table-hover">
        ...
        <tr>
          <th scope="col-11">제목</th>
          <th scope="col-1">작성자</th>
        </tr>
        </thead>
        <tbody>
          <tr v-for="article in column_article" :key="article.pk">
            <td>
              <router-link class="text-decoration-none" style="color:black;" 
                :to="{ name: 'article', params: {articlePk: article.pk} }">
                {{ article.title }}
              </router-link>
              <!-- <span class="badge bg-primary rounded-pill">{{ article.comment_count }}</span>               -->
            </td>
            <td>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
        <button class="btn" style="background-color: MidnightBlue;" type="button"><router-link :to="{ name: 'columnnew' }" class="text-decoration-none" style="color:white;">글쓰기</router-link></button>
      </div>
      ...
      <nav aria-label="Page navigation example">
        <ul class="pagination justify-content-center">
          ...
          <li class="page-item"><a class="page-link" style="color:DarkOrange;" href="#">1</a></li>
