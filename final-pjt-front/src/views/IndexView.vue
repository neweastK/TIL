    ...
    <div>
     <div class='container'>
        ...
        <div class='row justify-content-center mx-5'>
            <p v-show=false>{{ currentUser }}</p>
            <p v-show=false>{{ profile }}</p>
            <p v-show=false>{{ fromwatch }}</p>
          ...
          <br>
          <hr>
          <p v-if="inwatch">최근 {{ profile.watch_movie[this.profile.watch_movie.length-1].title }}를 본 당신! 이 영화는 어떠신가요?</p>
          ...
          <p v-else>영화에 하트를 눌러주세요! 더 정확한 추천을 받을 수 있습니다!</p>
          <movie-card
