...
- DP 배열의 인덱스가 과도하게 크다면 `좌표압축` 고려
  - 좌표압축 : 해쉬를 사용하여 dp 배열의 인덱스를 value로 나타내는 object 생성
...
> 해를 찾아가는 도중, 지금의 경로가 해가 될 것 같지 않으면 그 경로를 더 이상 가지 않고 되돌아가는 알고리즘
>
> 즉, 불필요한 부분은 끝까지 탐색하지 않고 가지치기하는 것
...
- DFS와의 차이는?
  - DFS는 끝이 나올 때까지 한 방향으로 탐색
  - 방향을 잘못 선정할 경우 손해가 매우 큼
  - 백트래킹을 활용해서 해가 나오지 않는 경로라면 끝까지 탐색하지 않도록 최적화
- 유망성 판단
  - 해가 될 가능성이 있다면?
    - 유망하다(promising)고 판단
  - 해가 될 가능성이 없다면?
    - 그 노드의 부모 노드로 돌아가 다음 자식 노드를 탐색(Backtracking)
...
>
> - 최댓값, 최솟값을 쉽게 찾기 위한 자료구조
> - 완전 이진 트리 : 단말노드를 제외한 모든 노드에서 자식들이 꽉 채워진 이진트리
...
- 배열의 인덱스는 각 노드의 번호
  - 배열[인덱스]에 해당하는 값이 노드가 갖고 있는 값
- i번 노드의 관계
  - i번 노드의 부모 노드 : a[(i-1)//2]
  - i번 노드의 왼쪽 자식 노드 : a[(i*2)+1]
  - i번 노드의 오른쪽 자식 노드 : a[(i*2)+2]



#### :heavy_check_mark: point!

- 모든 노드에 대해서 부모의 값이 자식의 값보다 항상 크거나 항상 작다
  - 부모가 더 작다면 최소힙, 부모가 더 크다면 최대힙
- 최대힙
  - 삽입과정
    - 가장 말단에 있는 노드의 자식 노드로 새로운 값 삽입
    - 이후, 부모노드와 비교하며 재구성
      - 부모노드와 비교해서 자식노드가 더 크다면 교환
      - 자식노드가 더 작다면 모든 작업 종료

  - 삭제과정
    - 루트 노드를 삭제하고 마지막 원소를 루트로 이동
    - 자식노드와 루트로 이동한 노드의 값 비교
      - 자식노드가 더 크다면 교환
      - 자식노드가 더 작다면 작업 종료
- 최소힙
  - 삽입과정
    - 최하단 레벨의 좌측에 새로운 값 삽입
    - 부모노드보다 작을 경우 위치 교환 (반복)

  - 삭제과정
    - 루트 노드 추출 (추출된 노드가 삭제된 노드)
    - 다시 최소힙 만족하도록 만들기
      - 최하단 레벨의 우측 노드부터 루트 노드로 이동
      - 이동한 노드가 자식 노드보다 클 경우 두 노드의 위치 교환

- heapq.heappush() + heapq.heappop() 보다 heapq.heappushpop()이 더 효율적임!
...
- push 하고 pop하는 것보다 heapq.heappushpop 한번에 하는게 더 효율적
...
> 두 노드의 공통 조상 중 가장 가까운 조상을 찾아내는 알고리즘
>
> 기본 알고리즘의 경우 시간 복잡도는 O(n), 개선된 알고리즘은 O(logn)



#### :heavy_check_mark: point!

- [가장 좋았던 강의](https://www.youtube.com/watch?v=GTYSPwz5ByU) 
- 트리 자료구조에 대해 얼마나 잘 알고 있는가가 중요
- 이진법 + DP + 트리 자료구조 결합의 알고리즘
- 각 노드별 이 노드가 공통 조상 노드가 맞는지를 확인하며 알고리즘 진행
  - 맞다면 더 낮은 공통 노드를 찾기 위해 계속 탐색
  - 아니라면, 해당 노드를 기준으로 다시 조상노드 탐색
    - 이 때, 2^k 번째 조상에서 멈췄다면, 해당 조상의 2^(k-1)번째 조상부터 탐색
    - 왜냐하면, 2^k번째 조상의 2^k번째 조상은 멈추기 바로 직전이었던 2^(k+1)번째 조상과 같기 때문
    - 즉, 이미 탐색했으므로 그 아래 조상부터 찾으면 됨
- 깊이를 같게 할 때, 한칸씩 이동하는게 아닌 2의 제곱씩 이동
  - 이를 위해, 깊이 차이를 2진수로 바꿔서 2의 제곱씩 이동할 때의 방법 확인
- 각 노드의 조상 노드는 DP를 활용하여 구현
  - 점화식 : dp[n]\[k] = dp[dp[n]\[k-1]]\[k-1]
  - 열부터 채워나가야함
    - dp[n]\[0] ~ dp[n]\[k] 까지 채운 후 dp[1] 부분 채워야함



#### :arrow_forward: 동작 과정

1. 모든 노드에 대한 깊이 계산하는 함수 구현
2. 부모 관계를 나타낼 dp 활용 배열 구현
3. 최소 공통 조상을 찾을 두 노드 확인 및 함수 구현
   1. 두 노드의 깊이가 동일해지도록 거슬러 올라감
   2. 부모가 같아질 때까지 반복적으로 두 노드의 조상노드 확인
      1. 루트 노드부터 내려오는 순으로 확인
      2. 부모가 같아지면 계속 탐색
      3. 부모가 다르면 값 갱신
         1. 이전에 같았던 부모와 현재 값이 다른 부모 노드 사이에 정답이 있기 때문
   3. 모든 LCA(a,b) 연산에 대해 3~4번 과정 반복
...
def binary_search(start,end,target):
    # 처음에 했던 우려
    # start가 end를 넘어갔는데 그 값이 우리가 원하는 값이면 어떡하지?
    # 그럴리가 없다. end보다 위는 이미 조건에 해당하지 않는다는 것
    # end보다 뒤에있는 값이 조건에 맞았다면, 애초에 end가 거기로 지정되었을리 없음
    if start>end:
        return start
    mid = (start+end)//2
...
- 두 수 a,b(a>b)에 대하여, a를 b로 나눈 나머지가 r이라 한다면
  - a와 b의 최대공약수는 b와 r의 최대공약수와 같다
  - r이 0일 때의 b가 두 수의 최대공약수
...
- 소수의 배수는 약수로 무조건 소수를 포함하게 된다
- 따라서, 소수의 배수는 절대 소수가 될 수 없다
- 이를 이용하여, 가장 작은 소수부터 해당 소수의 배수를 모두 지우는 것
  - 이 과정을 반복하면 결국 소수만 남게됨
