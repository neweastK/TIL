...
1. 인라인 - body의 html 태그에 style 속성을 지정하여 스타일 설정
   ex) \<h1 style="color: blue; font-size: 10px;">Hello\</h1>

2. 내부참조 - head 부분에서 스타일 지정하기
   ex)
...
3. 외부 참조 - CSS 파일을 새로 만들고 스타일 지정후 불러오기
...
- 기본 선택자 
  ...
  - `요소.클래스 {}` 로 요소 내에서 해당 클래스를 가진 요소들만 선택 가능
  
  - 아이디 선택자 : `#아이디이름` 으로 지정하고 아이디에 해당 이름을 지정하면 스타일이 자동 지정됨.
                                  ※ 같은 아이디를 여러번 사용해도 동작하지만, 단일 id를 사용하는 것을 권장
  
  - 속성 선택자 : 태그에 들어간 속성을 기준으로 스타일을 지정함. `[속성명]` 의 형식으로 스타일 지정
                              때문에 주로 요소 속성자와 함께 쓰인다. ex) `요소(태그)명[속성명]`
...
- 색상 단위
  ...
  - a는 alpha(투명도) 1로 갈수록 진해짐.
    rgba(x,y,z,a) / hsla(x,y,z,a) 등의 방법으로 사용
...
- 인접 형제 결합자
  ...
  - p + span  :  p와 같은 라인에 있으면서 바로 뒤에 있는 span
    바로 뒤에 span 이 없으면 당연히 적용 안됨. 바로 뒤에 다른 요소가 있고 그 다음에 바로 형제 요소가 있어도 적용 안됨. 무조건 바로 뒤여야만 적용
    
    

#### 선택자

- 선택자에는 요소, 클래스, id(#), attribute([]), 자식 결합자(>), 형제 결합자(~,+) 등이 들어 갈 수 있음.
...
- 블럭이면 한칸을 전부 차지하고, 인라인이면 한칸의 조금씩 차지한다.
...
- border : 박스를 이루는 테두리 영역
                  border-width : 테두리 두께, border-style : 선 스타일, border-color : 선 색깔 (이 순서대로 shorthand 가능)

- padding : 테두리 안쪽의 내부 여백, **요소에 적용된 배경색**, 이미지는 padding까지 적용 (content와 border 사이 공간)

- margin : 테두리 바깥의 외부 여백. **배경색을 지정할 수 없음**.
  하나의 박스모델과 서로 다른 박스모델 간 겹치는 부분이 없도록 하기 위함.
  상하좌우의 여백을 설정할 수 있음. (shorthand 설정도 가능:위 > 오른쪽 > 아래 > 왼쪽 순서)
  ...
  #margin이 하나인 경우에는 전체가 10px로 지정됨.
...
- block 
  ...
  - 만약 요소의 너비가 전체를 가질 수 없다면 나머지 부분은 margin이 자동으로 부여됨
...
- inline
  ...
  - content 너비만큼 가로 폭을 차지함(인라인의 기본 너비는 콘텐츠 길이만큼)
  - **width, height을 지정할 수 없으며** 상하 margin도 지정할 수 없다. 
  - 상하 여백은 line-height로 지정하며 요소의 크기보다 크면 다른 영역을 침범하게 됨.
...
- 문서 상에서 요소 위치를 지정
- 특정 요소의 위치 기준점은 좌측 상단! 좌측 상단 모서리의 위치를 의미하는 것
- `position: 포지션종류;` 로 설정



- static : 모든 태그의 기본 값, HTML에 따른 위치
...
- fixed : 고정 위치
  - 부모 요소와 관계없이 viewport를 기준으로 위치 차지
  - 요소를 일반적인 문서 흐름에서 제거 후 레이아웃에 공간을 차지하지 않음.

- 어떻게 이동시키는지?

  - relative, absolute, fixed 는 모두 top, left, right, bottom 의 좌표 프로퍼티를 사용하여 이동 가능
  - 예시
